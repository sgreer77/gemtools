...
def refine_bcs(**kwargs):
	...
	bed_df = pd.read_table(bed_in, sep="\t")
	bed_df.columns = ['chrom','start','stop','name','status']
	
	bed_df['bcs'] = bed_df.apply(lambda row: get_barcode_ids(bam_open,str(row['chrom']),int(row['start']),int(row['end']),MIN_MAPQ),axis=1)

...
def pipeline_from_parsed_args(args):
	...
	if args.tool=="assign_sv_haps":
		pipeline = assign_sv_haps(sv=args.infile, window=args.window_size, vcf_control=args.vcf_control, vcf_test=args.vcf_test, out=args.outfile, bcs=args.bc_select)
...
Usage:   gemtools -T assign_sv_haps [OPTIONS] -i <out.shared.txt> -c <LR_control.vcf.gz> -t <LR_test.vcf.gz> -o <out.haps.txt> -q <shared/select>
Input:
...
Options:
	-w  window size around breakpoint in which to look for phased SNVs; if not specified, will use windows already generated in input file (-i)
			"""
			sys.exit(1)
		if not (args.infile or args.outfile or args.vcf_control or args.vcf_test or args.bcs):
			parser.error('Missing required input')

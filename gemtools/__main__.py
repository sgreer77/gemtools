...
def get_option_parser():
	...
	parser.add_argument("-a", "--output_sub", metavar="FILE",
		dest="out_sub",
		help="Name of subset output file")
...
def pipeline_from_parsed_args(args):
	...
	if args.tool=="get_shared_bcs":
		pipeline = get_shared_bcs(sv=args.infile, bam=args.bam, out=args.outfile, out_sub=args.out_sub)
	...
	if args.tool=="refine_bcs":
		pipeline = refine_bcs(bed_in=args.infile, out=args.outfile, bam=args.bam)
	return pipeline
...
Options:
	-a  output file: Subset of output file (-o), so user can refine/replace shared barcode entries
			"""
			sys.exit(1)
...
Usage:   gemtools -T assign_sv_haps [OPTIONS] -i <out.shared.txt> -c <LR_control.vcf.gz> -t <LR_test.vcf.gz> -o <out.haps.txt>
Input:
...
Usage:   gemtools -T count_bcs [OPTIONS] -i <out.shared.txt> -b <LR.bam> -s <sv_name> -q <all/shared> -o <out.bc_count.txt>
Input:
	-i  output file from 'get_shared_bcs' tool
	-b  bam file generated by Long Ranger
	-s  name of the SV(s) to check; can be a comma-separated list

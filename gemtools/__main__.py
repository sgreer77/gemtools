...
##########################################################################################		
	if args.tool=="set_hap_window":
		#print "gemtools -T set_hap_window -i [LR_input.bedpe] -w [window_size] -o [out.bedpe]"
		if args.help:
			print """
Tool:	gemtools -T set_hap_window
Summary: Generate windows around SV breakpoints for SV haplotype analysis\n
...
##########################################################################################
	if args.tool=="get_shared_bcs":
		#print "gemtools -T get_shared_bcs -i [out.bed] -b [LR_bam_file] -o [out.shared]"
		if args.help:
			print """
...
##########################################################################################
	if args.tool=="plot_hmw":
		#print "gemtools -T plot_hmw -i [out.bc_count] -o [out.png]"
		if args.help:
			print """
Tool:	gemtools -T plot_hmw
Summary: Generate a plot of the mapping locations of reads with each barcode\n
Usage:   gemtools -T plot_hmw -i <output.bc_count.txt> -o <output.png>
Input:
...
##########################################################################################
	if args.tool=="set_bc_window":
		#print "gemtools -T bedpe2window -i [LR_input.bedpe] -w [window_size] -o [out.bed]"
		if args.help:
			print """
Tool:	gemtools -T set_bc_regions
Summary: Generate windows for SV analysis\n
Usage:   gemtools -T set_bc_regions [OPTIONS] -i <LR_input.bedpe> -w <window_size> -m <region_mode: auto or window -o <out.bed>>
Input:
	-i bedpe file of SV breakpoints; this is typically the Long Ranger output: large_sv_calls.bedpe OR large_sv_candidates.bedpe

...
def plot_haps_and_blocks(**kwargs):
	...
	plotFunc_blockvars = robj.r("""
		function(df_var,df_blk,b,e,xname,outplot)	{
			...
			# plot variants
			points(y_hap1~pos_mod, data=df_var[df_var$allele_2==1 & df_var$phase_status=='phased',], pch=4, cex=1.6, col="#663D54")
			points(y_hap2~pos_mod, data=df_var[df_var$allele_1==1 & df_var$phase_status=='phased',], pch=4, cex=1.6, col="#2B93CE")
			#points(y_hap1~pos_mod, data=df_var[df_var$allele_1==1 & df_var$phase_status=='phased' & df_var$pos==11468296,], pch=19, cex=1.6, col="red", bg="red")
